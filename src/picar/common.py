...
"""This module contains code that is used by many nodes. Shall avoid redundancy.
...
def set_param(param_name, param_value):
    """Sets a ROS parameter and prints logging info.
    Args:
...
def get_config_file_path(config_dir_name, file_name):
    ...
    if not os.path.isfile(default_config_file_path):
        rospy.logwarn(
            "No default configuration file in "
    ...
    if not os.path.isfile(config_file_path):
        ...
        if not default_is_file:
            return None
        rospy.logwarn(
            "Falling back to default config file in "
...
def get_camera_info():
    """
    Waits for a camera_info message and returns it.
    Returns: camera_info message object with camera parameters.
    """
    rospy.loginfo(
        "[{}] "
    ...
    if camera_info.K[0] == 0.0:
        ...
        rospy.signal_shutdown(
            "Missing intrinsic calibration data.\n"
            "Shutting down!")
    return camera_info


class FisheyeCameraModel(object):
    """Fisheye adaption of the PinholeCameraModel from the image_geomtry ROS package.

    See http://docs.ros.org/diamondback/api/image_geometry/html/c++/classimage__geometry_1_1PinholeCameraModel.html
    ...
    def from_camera_info(self, camera_info):
        """Creates camera model from a camera_info message.

        :type camera_info: CameraInfo
        """
        self.K = numpy.resize(
            camera_info.K, [3, 3]).astype(dtype=numpy.float32)
        self.D = numpy.resize(
            camera_info.D, [1, 4]).astype(dtype=numpy.float32)
        self.P = numpy.resize(
            camera_info.P, [3, 4]).astype(dtype=numpy.float32)
        self.P = self.P[:3, :3]
    ...
    def undistort_point(self, point):
        """Undistorts the coordinates of a pixel based on camera_info.
        Args:
            point: Pixel coordinates

        Returns: undistorted coordinates of the input pixel.

        """
        src = numpy.array(point, dtype=numpy.float32)
        src.resize([1, 1, 2])
        dst = cv2.fisheye.undistortPoints(
            src,
            self.K,
            self.D,
            R=self.R,
    ...
    def undistort_image(self, image):
        """Undistorts a whole image.

        Consider using undistort_point() instead, due to high computational costs
        of undistorting a whole image.
